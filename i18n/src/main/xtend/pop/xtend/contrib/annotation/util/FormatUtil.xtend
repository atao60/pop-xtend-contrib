...
class FormatUtil {
    ...
    /**
     * Rules to escape format element.
     * 
     * This engine keep a behavior close to the original pattern one of MessageFormat with 2 changes:
     * <li> a standalone single quote is interpreted as a single quote (never needs to be doubled), 
     ...
     *      quotes in translated strings where the original version doesn't have them. </quote>
     *   
     ...
     *         Otherwise, a standalone single quote will be interpreted as a single quote<quote>
     * But those rules are not so easy to master. And they use named parameters, not positionned ones. 
     * 
     * 
     */
